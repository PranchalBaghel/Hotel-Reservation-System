...
import hrs.entity.Booking;
import hrs.entity.Room;
		...
		try {
			...
			if (rs.next()) {
				...
				PreparedStatement pst1 = con.prepareStatement(
						"insert into book(customer_name , room_id , check_in , check_out) values(?,?,?,?)");
...
//show all booking
	public void ShowAllBooking() {
		try {
			...
			ResultSet rs = pst.executeQuery();
			while(rs.next()) {
				System.out.println("Book ID : "+rs.getInt("id") + " ," + " Customer Name : "+ rs.getString("customer_name") + " , " + " Room Id : "+rs.getInt("room_id") + " , " + " checkIn : "+rs.getDate("check_in") + " , " + " checkOut : "+rs.getDate("check_out"));
